...
#
# @(#) $Revision: 1.18 $
# @(#) $Id: rsyncfrom.sh,v 1.18 2014/01/24 07:45:51 chongo Exp $
# @(#) $Source: /usr/local/src/bin/rsynchost/RCS/rsyncfrom.sh,v $
#
...
# parse args
#
USAGE="usage: $0 [-C] [-e] [-k] [-n] [-p num] [-q] [-v] [-x] [-z] [user@]host dir
...
#
# NOTE: exit 100 is used by rsyncto
#
...
if [[ ! -d "$DOTDOT" ]]; then
    echo "$0: count not create parent directory: $DOTDOT" 1>&2
    exit 110
fi
export DOTDOT
...
status="$?"
if [[ "$status" -ne "0" ]]; then
    echo "$0: cannot cd $DOTDIR" 1>&2
    exit 111
fi

# get the real absolue path to ..
#
DOTDOTDIR=$($PWD_PROG)
export DOTDOTDIR
...
#
if [[ -n "$V_FLAG" ]]; then
    echo cd "$DOTDOTDIR;" $RSYNC_PROG $RSYNC_ARGS "$USERHOST:$DOTDOTDIR/$DEST" .
fi
if [[ -z "$N_FLAG" || -n "$V_FLAG" ]]; then
    eval $RSYNC_PROG $RSYNC_ARGS "$USERHOST:$DOTDOTDIR/$DEST" .
    status="$?"
else
    status="0"
fi
